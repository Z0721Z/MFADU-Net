...
from nets import Unet, Double_Unet
from my_nets import RD_Unet, CD_Unet, RD_Unet_mulitfusion
...
if __name__ == '__main__':
    ...
    # model-name: DCSAU_Net,Unet,doubleUnet,my_unet
    ...
    parser.add_argument('--epoch', type=int, default=500, help='epoches')
    args = parser.parse_args()
    name = args.model_name
    os.makedirs(name=name + ' save_models/', exist_ok=True)
    ...
    if name == 'Unet':
        model_ft = Unet.Unet(num_classes=1)
        # model_ft = unet.U_Net(3, 1)
    if name == 'doubleUnet':
        model_ft = Double_Unet.Double_UNet()

    if name == 'my_unet':
        model_ft = RD_Unet.Double_UNet()
    ...
    ftime = data_time.strftime("%Y_%m_%d_%H")
