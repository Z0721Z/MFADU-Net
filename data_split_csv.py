...
def pre_csv(data_path, frac, filename):
    np.random.seed(42)
    ...
    df.to_csv('src/' + filename + '/' + 'test_train_data.csv', index=False)
...
if __name__ == '__main__':
    ...
    os.makedirs('src/' + args.filename, exist_ok=True)
    pre_csv(args.dataset, args.size, args.filename)
